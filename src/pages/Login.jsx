...
import { useGoogleLogin } from "@react-oauth/google"; // NAYA: Google login import
...
const Login = () => {
  ...
  // NAYA FUNCTION: Custom Google Login Trigger
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        const response = await fetch(
          "https://medilab-ai-backend.onrender.com/api/auth/google",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            // Ab hum backend ko access_token bhej rahe hain
        ...
        setErrorMessage("Server error during Google Login. Check backend.");
      } finally {
        setIsLoading(false);
      }
    },
    onError: () => setErrorMessage("Google Login was closed or failed."),
  });
        ...
        <div className="w-full md:w-1/2 p-10 flex flex-col justify-center bg-navy/50 relative">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {/* NAYA CODE: Social Login Buttons (Top) */}
              <div className="space-y-3 mb-6">
                {/* Asli Google Login Button */}
                <div className="w-full flex justify-center">
                  {/* Asli Custom Google Login Button */}
                  <button
                    type="button"
                ...
                <button
                  type="button"
                  onClick={() =>
                    alert("Microsoft login backend se connect karna baaki hai!")
                  }
                  className="w-full py-3 bg-navy-light border border-navy-lightest hover:border-mint/50 hover:bg-navy-light/80 text-white font-medium rounded-xl flex items-center justify-center gap-3 transition-all duration-300"
                >
